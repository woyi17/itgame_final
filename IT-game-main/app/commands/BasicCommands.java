...
public class BasicCommands {
	...
	/**
	 * Sends a game-over message to the front-end, which displays a full-screen
	 * result overlay. playerWon should be true if the human player won.
	 */
	public static void showGameOver(ActorRef out, boolean playerWon) {
		try {
			ObjectNode returnMessage = Json.newObject();
			returnMessage.put("messagetype", "gameOver");
			returnMessage.put("winner", playerWon ? "player" : "ai");
			if (altTell!=null) altTell.tell(returnMessage);
			else out.tell(returnMessage, out);
		} catch (Exception e) {
			e.printStackTrace();
